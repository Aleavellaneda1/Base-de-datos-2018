...
FROM  
		provincia INNER JOIN localidad ON provincia.prov_id = localidad.loc_id
				  INNER JOIN domicilio ON localidad.loc_id = domicilio.dom_id
				  INNER JOIN cliente ON domicilio.dom_id = cliente.dom_id
				  INNER JOIN fac_cab ON cliente.clie_id = fac_cab.clie_id



WHERE provincia.prov_id =2

articulo  INNER JOIN fac_det on articulo.art_id = fac_det.art_id 
				 INNER JOIN fac_cab  ON  fac_cab.fac_cab_id = fac_det.fac_det_id
...
SELECT art_desc as 'Descripcion articulo',  count(fac_det_cant)
...
WHERE
 fac_cab_fecha >= '2016-03-01'
AND fac_cab_fecha <= '2016-03-31'